...
「それは気になる」
「続きが読みたい」
と思うような、強く自然なフックにしてください。

問いかけ型に固定してはいけません。
記事内容に最も合う表現を自分で選んでください。
...
「でやんす」「やんす」などの言葉を、
記事の内容と文章に合う場合だけ自然に1回程度使用してください。

「やんすか？」を必ず使う必要はありません。

「やんす」を無理やり文章の最後に付けないでください。

例えば、

貧困率が29.4％→6.2％に？人口8500人の町で起きた変化、気になるでやんす！

270億パラメーターのAIが無料で動く？この性能はかなり注目でやんす。

三姉妹の舞台が全国へ？歌と宙乗りを組み合わせた演出、その理由が気になるでやんす！

日本代表入りへ大きく前進？今季の数字を見ると、期待したくなるでやんす！

のように、記事に合わせて自然に変化させてください。

ただし、毎回「気になるでやんす！」を使わないでください。
...
この記事について、
Xで読者が続きを読みたくなる「強いフック」を1行だけ作成してください。

問いかけ型に固定せず、
記事に最も合う自然な表現を選んでください。

「やんす」は自然に使える場合だけ使用してください。
`,
        },
      ],
      temperature: 0.9,
