...
type Props = {
  importanceWeekly: ImportanceItem[];
  importanceMonthly: ImportanceItem[];
...
export default function SoccerRankingSidebar({
  ...
  const [importanceMonthlyMode, setImportanceMonthlyMode] =
    useState(false);

  const [viewsMonthlyMode, setViewsMonthlyMode] =
    useState(false);

  const importanceRanking = importanceMonthlyMode
    ? importanceMonthly
    : importanceWeekly;

  const viewsRanking = viewsMonthlyMode
    ? viewsMonthly
    : viewsWeekly;
      ...
      <div className="rounded-3xl border border-gray-200 bg-white p-8 shadow-lg">
        <div className="mb-8">
          ...
          <div className="mt-1 text-center text-lg font-black text-slate-400">
            {importanceMonthlyMode ? "月間" : "週間"}
          </div>
        </div>

        <div className="space-y-5">
          {importanceRanking.map((item, index) => (
            <Link
              key={item.id}
              href={`/news/${item.id}`}
        ...
        <div className="mt-6 border-t border-slate-100 pt-5 text-right">
          <button
            type="button"
            onClick={() =>
              setImportanceMonthlyMode(
                !importanceMonthlyMode
              )
            }
